    ...
    if (unicode_escape || 1 == jsondata->all_unicode || (jsondata->source_unicode && has_non_ascii)) {
        if (!jsondata->source_unicode && has_non_ascii) { // probably this is UTF-8 with unicode_escape
            ...
            if (tmpObject != NULL) {
                ...
                if (object) {
                    tmpObject = object;
                    object = PyUnicode_DecodeUnicodeEscape(PyBytes_AS_STRING(tmpObject), PyBytes_Size(tmpObject), NULL);
                    Py_XDECREF(tmpObject);
                }
            }
        } else {
...
/*
 * This function is an almost verbatim copy of PyBytes_Repr() from
